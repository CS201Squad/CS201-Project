...
<%
	EntireCourse e=(EntireCourse)request.getAttribute("entireCourse");
	HashMap<Integer, CourseProfessor> courses=e.get_CP();
%>
...
<body>
    ...
    <div class="row">
        ...
        <% if(courses.size()==0){%>
            ...
            <p id= "class-name">Sorry, no results were found.</p> 
        </div>
		<%} %>
        <% if(courses.size()>0){%>
        <div class="left">
            <div id="class-base">
                <p id="class-id"><%=e.get_prefix().toUpperCase()+" "+Integer.toString(e.get_courseNum()) %></p>
            </div>
        </div>
        <%} %>
        <div class = "right">
            <!-- Here the class name should be loaded-->
            <% if(courses.size()>0){%>
            <p id= "class-name"><%=e.get_name() %></p> 
            <%} %>
        </div>
    </div>
    <% if(courses.size()<0){%>
    <h3 id = "intro-text"> Professors</h3>
    <%}%>
				...
				%>
					<tr>
						<td>
							...
							<h4 class = "profName">
							<a href="CourseProfessorResult<%=params%>"><%=fname+" "+lname %></a>
							</h4>
						</td>
						<td>
